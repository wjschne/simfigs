...
```{r, eval = FALSE}
install.packages(c("tidyverse", "dplyr", "ggplot2", "readr", "targets", "tidyr", "ggtikz"))
```
...
```{targets some-globals, tar_globals = TRUE}
...
library(ggtext)
library(ggtikz)
tar_option_set(packages = c("extrafont", "lavaan", "tidyverse","patchwork", "mvtnorm", "simstandard","ggtext", "glue", "WJSmisc", "ggtikz"))
...
```{r}
pthreshold <- function(threshold = 85) {
  v_rxx <- round(c(seq(0.80, 0.95, 0.05), 0.98), 2)
  
d <- crossing(x = round(seq(55, 145, 0.5), 1), rxx = v_rxx, threshold = threshold) %>%
  mutate(see = 15 * sqrt(rxx - rxx ^ 2),
         mu = (x - 100) * rxx + 100,
         p = pnorm(threshold, mu, see)) %>%
  group_by(rxx) %>%
...
d_labels <- tibble(rxx = v_rxx) %>%
  mutate(x = c(69.6, 72.1, 74.5, 77.4, 79.9)) %>%
  left_join(d, by = c("rxx", "x"))

gp <- d %>%
  ggplot(aes(x,p)) +
  geom_line(aes(color = factor(rxx)), lwd = 1) +
  geom_vline(aes(xintercept = threshold), lty = 2, lwd = 1, color = "gray30") +
  ggtext::geom_richtext(
    aes(label = rxx_label,
        color = factor(rxx)),
    data = d_labels %>%
      mutate(rxx_label = WJSmisc::prob_label(v_rxx)),
    angle = -55,
    size = 4.5,
    label.colour = NA,
    fill = "#FFFFFF",
  ...
  scale_x_continuous("Observed Score", breaks = seq(40, 160, 15),
                     minor_breaks = seq(40, 160, 5),
                     expand = expansion()) +
  scale_y_continuous("Probability True Score < 85", expand = expansion(),
                     breaks = seq(0,1, 0.1),
                     labels = WJSmisc::prob_label,
                     limits = c(0,1)) +
  scale_color_viridis_d(begin = 0.2, end = 0.8) +
  theme_minimal(base_family = "Roboto Condensed", base_size = 20) +
  theme(legend.position = "none", plot.margin = unit(c(3,5,2,2), "mm")) +
  coord_fixed(ratio = 100, clip = "off", xlim = c(55,145))


}
pthreshold()

katex::katex_mathml("\\mu_X")
```
